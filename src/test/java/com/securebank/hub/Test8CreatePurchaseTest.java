...
public class Test8CreatePurchaseTest {
  ...
  @Test
  public void test8CreatePurchase() {
    ...
    // Wait for login page and login as ADMIN (needed for PURCHASE option)
    ...
    // Select PURCHASE from transaction type dropdown
    ...
    try {
      select.selectByVisibleText("Purchase");
    } catch (Exception e) {
      select.selectByValue("PURCHASE");
    }
    ...
    if (amountValue == null || amountValue.isEmpty() || !amountValue.contains("75")) {
      ((JavascriptExecutor) driver).executeScript(
        "var input = arguments[0];" +
        "var nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set;" +
        "nativeInputValueSetter.call(input, '75.00');" +
    ...
    try {
      java.util.List<WebElement> textInputs = driver.findElements(By.cssSelector(".transaction-form input[type='text']"));
      if (textInputs.size() > 0) {
        WebElement descriptionInput = textInputs.get(0);
        if (descriptionInput.isDisplayed()) {
          descriptionInput.click();
          descriptionInput.sendKeys("Test purchase transaction");
        }
      }
    } catch (Exception e) {
    ...
    System.out.println("Submitting purchase form...");
    ...
    try {
      ...
      if (successAlerts.size() > 0 && successAlerts.get(0).isDisplayed()) {
        ...
        assertTrue("Purchase should succeed", true);
        ...
        if (errorText.contains("Insufficient funds")) {
          ...
          throw new RuntimeException("Purchase failed with error: " + errorText);
        }
      }
      ...
      if (forms.size() == 0) {
        System.out.println("Form disappeared - purchase likely succeeded");
        assertTrue("Purchase likely succeeded (form disappeared)", true);
      } else {
        throw new RuntimeException("Purchase form still visible after timeout");
      }
    }
